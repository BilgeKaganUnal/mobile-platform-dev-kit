...
class ATTService {
  ...
  async initialize(): Promise<TrackingPermissionResult> {
    try {
      logger.info('ATTService: Initializing ATT permissions');
      const result = await trackingPermissions.initialize();
      logger.info('ATTService: ATT permissions initialized', { 
  ...
  async requestPermissions(): Promise<TrackingPermissionResult> {
    try {
      logger.info('ATTService: Requesting ATT permissions');
      const result = await trackingPermissions.requestPermissions();
      logger.info('ATTService: ATT permissions request completed', { 
        status: result.status, 
        canTrack: result.canTrack 
      });
      return result;
  ...
  reset(): void {
    ...
    logger.info('ATTService: Service reset');
  }
}
